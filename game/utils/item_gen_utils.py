...
def place_items(game_board, loot_wave_num, number_items, consumable_count, upgrade_count, gun_count, money_count):
    ...
    for index in range(number_items):
        ...
        # if (x,y) collided with an already existing object too many times, the object will not spawn
        object_collided = 0
        while game_board.partition.find_object_hitbox(dummy_hitbox):
            object_collided += 1
            potential_x = max(min(random.gauss(half_width, half_width * .005), game_board.circle_radius - 1), 1)
            potential_y = max(min(random.gauss(half_width, half_width * .005), game_board.circle_radius - 1), 1)
            dummy_hitbox = Hitbox(5, 5, (potential_x, potential_y))
            # if item spawns outside kill boundary or there is not enough room, dont spawn it
            if (potential_x - GameStats.game_board_width/2)**2 + (potential_y - GameStats.game_board_height/2)**2 > game_board.circle_radius**2 or object_collided > 10:
                break
        if (potential_x - GameStats.game_board_width/2)**2 + (potential_y - GameStats.game_board_height/2)**2 > game_board.circle_radius**2 or object_collided > 10:
            continue
        item, consumable_count, upgrade_count, gun_count, money_count = \
            pick_item(potential_x, potential_y, consumable_count, upgrade_count, gun_count, money_count, loot_wave_num=loot_wave_num)
        if item is not None:
