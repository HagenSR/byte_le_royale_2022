...
def load_collidables(player, gameboard):
    partition_x = gameboard.partition.find_row(player.shooter.hitbox.position[0])
    partition_y = gameboard.partition.find_column(player.shooter.hitbox.position[1])
    collidables = {}
    if 0 <= player.shooter.heading < (math.pi/2) or player.shooter.heading == (2*math.pi):
        ...
        for x in range(partition_x, gameboard.partition.get_partitions_wide()):
            for y in range(partition_y, gameboard.partition.get_partitions_tall()):
                for z in gameboard.partition.get_partition_objects(x, y):
                    collidables[z] = 0
...
def sort_objects(player, collidables, gun):
    for collidable in list(collidables.keys()):
        dist = math.sqrt(((collidable.hitbox.position[0] - player.shooter.hitbox.position[0]) ** 2)
...
def determine_collision(player, collidables, slope, gun):
    ...
    ray_x_limit = (player.shooter.hitbox.position[0] + (gun.range
        * math.cos(player.shooter.heading)))
    ray_y_limit = (-player.shooter.hitbox.position[1] - (gun.range
        * math.sin(player.shooter.heading)))
    ...
    for collidable in collidables.keys():
        ...
        upper_y = -collidable.hitbox.position[1] + (collidable.hitbox.height / 2)
        lower_y = -collidable.hitbox.position[1] - (collidable.hitbox.height / 2)
        ...
        if (lower_y <= ray_y <= upper_y) and (left_x <= ray_x <= right_x):
            ...
            ray = Ray(player.shooter.hitbox.position, endpoint, collidable, gun.damage)
            break
